...
# ----------------------------
# Try existing session from global state
session = st.session_state.get("session")

# Fallback: show login form if missing
if not session:
...
with tabs[1]:
    ...
    for req in incoming:
        ...
        c1.write(f"**{name}**  \n`@{(prof[0]['username'] if prof else from_id[:8])}`")
    ...
    for req in outgoing:
        ...
        st.write(f"Sent to **{name}**  (`@{(prof[0]['username'] if prof else to_id[:8])}`) — *pending*")
